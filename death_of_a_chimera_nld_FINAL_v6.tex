...
engineering criterion at $t_{\mathrm{graze}} = 83.2$\,s. The deployed supervisor's
...
Replaying the deployed detector over the same unsupervised trajectories and
...
dynamical invariance of on-manifold initial conditions under the deployed integrator;
...
\textbf{Scope.} We state plainly what this paper does and does not settle about
the prolongation. Its objective is the empirical one, completed here: to establish
that the factor is real, fixed at $\approx 3.2\times$, and $N$-independent over a
$16\times$ range; to exclude additive collective noise, breath-locked
multiplicative noise, and the off-manifold Watanabe--Strogatz constants by direct
test; and to localize the surviving mechanism to finite-$N$ sampling of the
collective flow on the Poisson submanifold, acting in the slow saddle passages
where exit from the ghost is decided. Setting these bounds --- which any analytic
theory must now respect --- is the deliberate end point of the present work. The
complementary task, the derivation of the state-dependent diffusion tensor that
finite-$N$ sampling induces on the reduced flow near the saddle and the
backward-Kolmogorov first-passage calculation it feeds, we leave deliberately to
dedicated stochastic analysis; it is a self-contained problem in its own right,
...
sizes, the deployed phase lag ($\beta = 0.05$, with the phenomenology reproduced at
...
The synthesizer that posed the question also takes the answer. Because the
operating corner has no attractor, its renowned stability was never the
oscillators' to give --- it is manufactured entirely by the supervisor, and must
remain so at every $N$. What the dynamics changes is how that supervisor should
watch: the deployed two-second sustain rule fires on grazes the chimera survives
$69$--$97\%$ of the time, re-seeding a state that was not dead. An
absorption-grade detector --- one that waits out a recovery window before
declaring death, a hysteresis on the order parameter rather than a bare
threshold --- collapses that false-positive rate and intervenes an order of
magnitude less often for the same uptime. The prescription generalizes past this
voice to any coupled-oscillator audio engine driven near a breathing chimera:
gate restarts on absorption, not first passage. The discarded high-morph corner,
finally, is the one to reach for when genuine unattended persistence is wanted ---
it, and not the deployed corner, holds the only chimera that lives on its own.
...
To show the operating-corner phenomenology is not an artifact of the deployed phase
...
cross: the two deployed corners.}
\label{fig:cornermap}
\end{figure}
