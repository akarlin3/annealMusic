...
**v9.6 Chimera Synthesis Voice.** Turns the chimera phenomenon characterized in v9.5's research (`docs/CHIMERA_CHARACTERIZATION.md`) into a _playable_ synthesis voice: an emergent, **self-morphing timbre** with no LFO or envelope driving it. A new identical-ω `chimera` engine reuses the sine partial bank, but instead of the spread-ω drift loop the orchestrator runs a seeded **two-population Sakaguchi (Abrams–Strogatz) chimera** at control rate — one population locks while the other stays incoherent, and that asymmetry, mapped through the _existing_ `fusion.ts` `m_i` gain law, reinforces one band of partials over the other. As the chimera breathes (and occasionally swaps which population leads), the reinforced band moves and the spectral centroid morphs on its own. The spread-ω voices are **bit-identical** (the new mode is additive, gated behind voice selection).

**Honest framing (the bar v9.5's research set):** the chimera is a **seeded, supervised emergent** mode, not a spontaneous one. From random phases the basin is ~0, so the voice seeds the canonical state (pop 1 a synchronized cluster, pop 2 incoherent); and even seeded, a fraction of seeds collapse to global sync. The shippable part is the **supervisor**: it detects collapse and smoothly re-perturbs back into the basin, so the voice keeps morphing over a long session. Measured: at the gentle default (Intensity 0.2, A≈0.44) a **30-minute render stays in-chimera ~96%** with ~20 re-perturbations; a deliberately collapse-prone seed (bare fracAlive 0.47 at A=0.2) is lifted to >0.8 and keeps morphing. Re-perturbations are bounded by an output gain-slew limiter, so they read as a gentle timbral shift, never a click.

### Added

- **Pure chimera core (`src/audio/chimera.ts`):** a deterministic port of the verified Build-B probe — the two-population RK4 mean-field Sakaguchi step, per-population order parameters, the canonical seed (pop 1 cluster, pop 2 incoherent), the collapse-detector helpers (`chimeraSplit` / `isChimeraAlive` / `lockedPopulation`), and the `intensityToA` basin↔morph mapping (`A = 0.5 − 0.3·intensity`, clamped to [0.2, 0.5]). No audio-thread state; randomness only via an injected rng. The `chimeraFusionGains` bridge maps the two populations' coherence onto per-partial gains by **reusing `fusion.ts` `fusionMultiplier`** (no fusion math duplicated): the locked band aligned with the global mean field is reinforced; the incoherent band's `R → 0` scales the reshaping amount to ~0 so it stays at baseline.
- **Control-rate driver + supervisor (`src/audio/chimeraVoice.ts`):** `ChimeraVoice` seeds the chimera, advances the integrator per tick, and returns the bank's fusion gains, with a collapse-detect → re-seed supervisor and a per-tick output gain-slew limiter (`MAX_GAIN_SLEW`). Deterministic under an injected rng; default Np=64 per population (basin ~92%).
- **`chimera` engine:** `ChimeraEngine` reuses the sine partial bank verbatim (`SineEngine` now takes an `id`), registered in the engine registry/order/labels/URL-namespace; the orchestrator drift loop branches to the chimera driver when this engine is active and early-returns, leaving the spread-ω `driftStep` path untouched. New `chimeraIntensity` shared param (0..1, default gentle ≈0.2). New **Chimera Drift** preset in Ambient & Space.
- **Tests:** the core reproduces the probe's reference r/split numbers bit-for-bit (canonical regime, seed 9000); the gain bridge's reuse + band split; voice determinism and a real spectral-centroid morph through the 6-partial bank; the supervisor beats the bare-core baseline and holds the default ≥90% over a 10-minute run with no gain step beyond the slew cap; and the intensity control's morph monotonicity (≈54 → 98 → 118 Hz at Intensity 0.1/0.4/0.7), musical bounds (12–24% of the mean centroid), and smoothness across the range. All prior tests green; spread-ω engine tests unchanged.
- **Docs:** README **Engines** section (the chimera voice, the Intensity control, the honest seeded/supervised framing).
