...
| v7.7 ✅ | **v7 Closeout** — Research Collaboration Arc       | Closes the v7 Research Collaboration Arc. Delivers domain-specific onboarding and publishing manuals for ICAD and clinical researchers, five drop-in partnership templates (IRB consent, NSF/NIH DMP, Software Collaboration Agreement, Citation Guide, Supplementary Materials Checklists), legal posture declarations, stability API commitments, and final v7.7.0 release. |
| v8.0 ✅ | **v8.0 Audit + Plan**                              | The diagnosis slice. Establishes baseline diagnostic instrumentation and delivers the comprehensive primary audit reports and sequenced roadmaps for the entire subsequent v8 refactoring arc (v8.1–v8.5). |
| v8.1 ✅ | **v8.1 Code Health** — Dependency Upgrades         | Safely upgraded client stack (TypeScript 6, Vite 8, React 19, React Router 7, Tailwind 4, Zustand 5, Lucide-React 1.17) and backend, unified parameter schemas under a single canonical compile-time `schema/manifest.json` source of truth, eliminated dead exports and duplicate exports across client and backend files, and enforced uniform type safety to secure a strict 98.68% frontend type-coverage benchmark. See [docs/v8/PINS.md](v8/PINS.md). |
| v8.2 ✅ | **v8.2 Performance Pass**                          | Substantial wins across cold start, runtime CPU, mobile thermals, and bundle sizes. Route-level lazy loading and Dynamic Yjs/CRDT code-splitting reduced eager bundles by >250KB, and WebGL visualizer uniform update caching eliminated redundant WebGL driver state calls. Karplus-Strong & Waveguide waveguide delay line index calculations bypassed slow float modulo arithmetic. Modal resonator synthesis bank flattened to parallel Float32Array typed buffers inside AudioWorklet. Spaced heart rate BLE GATT updates to at most once per 200ms with a leak-proof queueing mechanism, preserving uncorrupted HRV. Clamped loop buffer allocations to exactly 60 seconds. Shipped with 100% test suite parity. See [docs/v8.2-PLAN.md](v8.2-PLAN.md). |
| v8.3 ✅ | **v8.3 Observability & Safety**                  | Implemented robust, privacy-first diagnostic error reporting and hardening constraints. Features zero-telemetry first-run glassmorphic consent modals, aggressive client-side PII and dynamic slug scrubbing, and self-hosted error collection. Hardens security boundaries via strict route rate limits (ORCID checks & gallery search), selective HTML Content-Security-Policy (CSP) headers, and UNIX sandboxed execution memory limits (MAX_MEM = 512MB) with graceful cross-platform ValueError support. |
| v8.4 ✅ | **v8.4 Architectural Refactoring**              | Consolidated six distinct render paths into a single environment-agnostic `RenderEngine`, resolved schema duplications in `schema/manifest.json`, unified transport bridges, modularized engines via composition typeguards, created native Capacitor shared plugin base, consolidated players under `SessionPlayer`, and introduced storage abstractions. |
| v8.5 ✅ | **v8.5 Testing & CI Hardening**                  | Achieved >85% statement coverage on Audio Engine Core and >95% on Schema / State Management, automated E2E smoke matrix using hermetic Playwright mocks, optimized CI runtime under 8 minutes, finalized v8 Retrospective and Stability Commitments, and released `v8.5.0` closing the version 8 maintenance arc. |
