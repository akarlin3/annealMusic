# AnnealMusic · Perceptual Experimentation Framework Guide

This document outlines the architecture, timing characteristics, IRB/privacy protocols, and data export schema for **AnnealMusic v5.6's Perceptual Experimentation Framework**, a specialized tool for music cognition and Music Information Retrieval (MIR) research.

---

## 1. Timing Precision & Architectural Characteristics

Psychoacoustic and cognitive behavioral studies require robust audio-stimulus scheduling. The AnnealMusic runner employs the following paradigm to achieve maximum reliability:
...
- **Matplotlib Visualizer Rendering:** By default, visual rendering animations (FFT spectrums, Kuramoto coupling visualizers) are **deactivated during active participant trials** to eliminate main-thread rendering bottlenecks and maximize scheduling stability, unless the stimulus configuration explicitly sets `"visualizer": true`.
...
```
[Subject Device (Browser VFS / Memory)]
            │
            ├─► Consent Verification Check
            ├─► Zero Persistent Cookies / Identifiers
            ├─► Wiping & Discarding on Abort / Exit
            │
            ▼
[Data Export Triggers]
      │
      ├─► Local ZIP Builder (Immediate Download)
      │
      └─► Manual POST (Participant reviews exact JSON payload before dispatch)
```
