...
- **Synchronization-Driven Spectral Fusion (v9.4)** — the headline synthesis idea: the Kuramoto order parameter `r` no longer only pulls the partials' _pitches_ together — it now reshapes their _timbre_. As the partial oscillators synchronize, each partial's amplitude is boosted in proportion to how well its phase aligns with the collective mean field, so the spectrum itself **fuses**: coherent partials reinforce into a more present, unified tone, while incoherent ones stay diffuse and shimmering. It's a single, controllable instrument parameter (`fusion`, off by default) coupling synchronization to spectrum rather than pitch. The effect is measured, not asserted: the FFT suite shows it is monotone in `r`, behavior-preserving at zero, and delivers ~3 dB of coherent reinforcement over the full sync range — honestly, a coherent-energy effect rather than a brightness tilt. Derivation in [docs/DSP_THEORY.md §1.6](docs/DSP_THEORY.md) and [docs/KURAMOTO.md](docs/KURAMOTO.md); proof in `src/audio/analysis/__tests__/fusion.test.ts`.
